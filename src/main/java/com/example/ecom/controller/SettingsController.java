...
public class SettingsController extends AbstractController<SettingService> {
    ...
    public ResponseEntity<CommonResponse<String>> updateGeneralSettings(HttpServletRequest request,
        ...
        return new ResponseEntity<CommonResponse<String>>(
                new CommonResponse<String>(true, null,
                        LanguageMessageKey.UPDATE_GENERAL_SETTINGS_SUCCESS, HttpStatus.OK.value(),
                        result.getViewPoints().get(SettingsResponse.class.getSimpleName()),
                        result.getEditable().get(SettingsResponse.class.getSimpleName())),
    ...
    public ResponseEntity<CommonResponse<String>> updateAccountInformation(
            HttpServletRequest request, @RequestBody AccountSetting accountSetting) {
        ValidationResult result = validateToken(request);
        service.updateAccountInformation(accountSetting, result.getLoginId());
        return new ResponseEntity<CommonResponse<String>>(
                new CommonResponse<String>(true, null,
                        LanguageMessageKey.UPDATE_ACCOUNT_SETTINGS_SUCCESS, HttpStatus.OK.value(),
                        result.getViewPoints().get(SettingsResponse.class.getSimpleName()),
                        result.getEditable().get(SettingsResponse.class.getSimpleName())),
                null, HttpStatus.OK.value());
    }

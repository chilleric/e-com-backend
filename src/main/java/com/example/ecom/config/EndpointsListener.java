...
import com.example.ecom.utils.StringUtils;
...
public class EndpointsListener implements ApplicationListener<ContextRefreshedEvent> {
  ...
  public void onApplicationEvent(ContextRefreshedEvent event) {
    ...
    if (permissions.size() == 0) {
      List<ObjectId> features = featureRepository.getFeatures(new HashMap<>(), "", 0, 0, "").get()
          .stream()
          .map(Feature::get_id).collect(Collectors.toList());
      ...
      Permission permission = new Permission(null, "super_admin_permission", userIds, features,
          DateFormat.getCurrentTime(), null, false, 0, permissionRepository.getViewPointSelect());

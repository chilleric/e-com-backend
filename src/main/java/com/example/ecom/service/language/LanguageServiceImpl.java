...
public class LanguageServiceImpl extends AbstractService<LanguageRepository> implements LanguageService {
    ...
    @Override
    public Optional<ListWrapperResponse<LanguageResponse>> getLanguages(Map<String, String> allParams, String keySort,
    ...
    public void addNewLanguage(LanguageRequest languageRequest) {
        ...
        List<Language> languagesName = repository
                .getLanguages(Map.ofEntries(entry("language", languageRequest.getLanguage())), "", 0, 0, "").get();
        if (languagesName.size() > 0) {
            Map<String, String> error = generateError(LanguageRequest.class);
            error.put("language", "Invalid config name");
            throw new InvalidRequestException(error, "Invalid config name");
        }
